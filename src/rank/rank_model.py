...
class Ranker(object):
    ...
    def __init__(self, bert_config_path, is_training, num_train_steps=None,
                 num_warmup_steps=None, learning_rate=1e-5, label_smooth=0.):
    ...
    def create_model(self):
        ...
        with tf.variable_scope("projection", reuse=tf.AUTO_REUSE):
            output_weights = tf.get_variable("output_weights", [num_labels, hidden_size],
                                             initializer=tf.truncated_normal_initializer(stddev=0.02))  # num_label=2
        ...
        if self.is_training:
            with tf.name_scope("train_op"):
                # 使用label_smoothing
                self.onehot_label_ids = tf.one_hot(self.label_ids, num_labels, dtype=tf.float32)
                self.loss = tf.reduce_mean(tf.losses.softmax_cross_entropy(
                    onehot_labels=self.onehot_label_ids,
                    logits=self.logits,
                ...
                #     logits=self.logits,
                #     labels=tf.reshape(self.label_ids, (-1, self.logits.shape[-1].value))))
    ...
    def train(self, sess, batch):
        ...
        _, loss, logits, prob = sess.run([self.train_op, self.loss, self.logits, self.probabilities],
                                         feed_dict=feed_dict)
